num_helper <- function(state_subset, col_num, num) {
    ...
    outcome_arr <- suppressWarnings(as.numeric(state_subset[, col_num]))
...
rankall <- function(outcome, num = "best") {
    ...
    df <- read.csv(myFileName, colClasses="character")
    ...
    state_arr <- sort(unique(df$State))
    arr_len <- length(state_arr)
    hospital <- rep("", arr_len)

    for(i in 1:arr_len) {
        # loop for each state
        state_subset <- df[df[, 7]==state_arr[i], ]
